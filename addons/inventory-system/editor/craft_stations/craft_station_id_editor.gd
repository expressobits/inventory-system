...
func _check_valid_id():
	...
	if ids_must_exist_in_database:
		valid = new_id == self.id or database.has_craft_station_type_id(new_id)
		warn_text = "CraftStation id does not exist in database!"
	else:
		valid = new_id == self.id or not database.has_craft_station_type_id(new_id)
		warn_text = "CraftStation id already exists in database!"
	if valid:
		id_line_edit.tooltip_text = ""
		button.tooltip_text = "Confirm"
		id_line_edit.modulate = Color.WHITE
