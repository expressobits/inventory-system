...
func _on_add_button_pressed():
	match new_type_option_button.get_selected_id():
		TYPE_BOOL:
			item.properties[line_edit.text] = false
		TYPE_INT:
			item.properties[line_edit.text] = 0
		TYPE_FLOAT:
			item.properties[line_edit.text] = 0.0
		TYPE_STRING:
			item.properties[line_edit.text] = ""
		TYPE_COLOR:
			item.properties[line_edit.text] = Color()
	loading_properties()
