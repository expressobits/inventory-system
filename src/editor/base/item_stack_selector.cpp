...
void ItemStackSelector::_ready() {
    // Create item ID editor
    item_id_editor = memnew(ItemIDEditor);
    item_id_editor->set_ids_must_exist_in_database(true);
    item_id_editor->set_h_size_flags(Control::SIZE_EXPAND_FILL);
    item_id_editor->connect("changed", Callable(this, "_on_product_id_spin_box_value_changed"));
...
void ItemStackSelector::_on_option_button_item_selected(int index) {
    ...
    if (item.is_valid()) {
        item_stack->set_item_id(item->get_id());
        if (item_id_editor->get_id() != item->get_id()) {
            item_id_editor->setup(database, item->get_id());
        }
        emit_signal("changed", item_stack);
    }
}
