...
class CraftStationTypesEditor : public BaseInventoryEditor {
	GDCLASS(CraftStationTypesEditor, BaseInventoryEditor);

private:
	// UI Components matching addon structure exactly
	HSplitContainer* hsplit_container;
	CraftStationTypeEditor* craft_station_type_editor;
	...
	void _on_craft_station_type_editor_changed(const Ref<CraftStationType>& craft_station_type);
...
public:
	...
	void load_craft_station_types();
	bool remove_current_data() override;
	void select(const Ref<CraftStationType>& craft_station_type);
