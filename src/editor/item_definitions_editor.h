...
class ItemDefinitionsEditor : public BaseInventoryEditor {
	GDCLASS(ItemDefinitionsEditor, BaseInventoryEditor);

private:
	// UI Components matching addon structure exactly
	HSplitContainer* hsplit_container;
	ItemDefinitionEditor* item_editor;
	...
	void _on_item_editor_changed(const Ref<ItemDefinition>& item_definition);
...
public:
	...
	void select(const Ref<ItemDefinition>& item);
