...
void LootEditor::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("_on_loot_item_changed", "loot_item"), &LootEditor::_on_loot_item_changed);
...
void LootEditor::_create_ui() {
	...
	loot_item_editor = memnew(LootItemEditor);
	items_right_vbox->add_child(loot_item_editor);
	loot_item_editor->set_v_size_flags(Control::SIZE_EXPAND_FILL);
	loot_item_editor->connect("changed", callable_mp(this, &LootEditor::_on_loot_item_changed));
	loot_item_editor->set_visible(false);
...
void LootEditor::_apply_theme() {
	...
	// Apply theme to loot item editor
	if (loot_item_editor) {
		loot_item_editor->set_editor_plugin(editor_plugin);
	}
...
void LootEditor::set_editor_plugin(EditorPlugin *p_plugin) {
	...
	if (loot_item_editor) {
		loot_item_editor->set_editor_plugin(p_plugin);
	}
	_apply_theme();
}
...
void LootEditor::_on_add_item_button_pressed() {
	...
	if (items_list->get_item_count() > 0) {
		...
		if (loot_item_editor) {
			loot_item_editor->load_item(new_item, database);
		}
	}
...
void LootEditor::_on_items_list_item_selected(int p_index) {
	Ref<LootItem> selected_item = _get_selected_loot_item();
	if (loot_item_editor) {
		loot_item_editor->load_item(selected_item, database);
	}
}
...
void LootEditor::_on_loot_item_changed(const Ref<LootItem> &p_loot_item) {
	if (p_loot_item.is_valid()) {
		_update_items_list();
		_update_total_weight();
		emit_signal("changed", current_loot);
	}
